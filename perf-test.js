    ...
    const res = ws.connect(url, params, function (socket) {
        ...
        socket.on('open', function open() {
            ...
            socket.on('message', function (msg) {
                ...
                if (Array.isArray(messageData) && messageData[0] === 'chat:message') {
                    console.log(`[VU-${vuId}] Received message: ${JSON.stringify(messageData[1])}`);
                    check(messageData[1], { 'Message received': (m) => m.message !== '' });
                }
            });
